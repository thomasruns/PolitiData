...
if (!file_exists('config.php'))
  die('Copy config.php-dist to config.php and add your database settings.');
...
// Let's make sure the table exists
$test = mysql_query('SELECT count(*) FROM candidate ') or die(mysql_error());
...
if($debugFlag!=1)
{
  ...
  exec("rm cache/*.zip cache/*.dta");

  //Download the latest file from the FEC and unzip it
  exec("curl ftp://ftp.fec.gov/FEC/cn12.zip -o cache/cn12.zip");
  exec("cd cache && unzip cn12.zip");

}

if(!file_exists("cache/foiacn.dta"))
{
